...
if (file.exists("spiegel_cont_urls_df.csv.gz")) {
    cont_urls_df <- read_csv2("spiegel_cont_urls_df.csv.gz", col_types = "ccccTc")
} else {
...
cont_urls_df <- bind_rows(url_df_list) %>% 
    ...
    mutate(timestamp = ymd_hm(timestamp, tz = "CET", quiet = T),
           ...
           url = paste0("http://www.spiegel.de", url)
           ) %>% 
    arrange(medium)


write_csv2(cont_urls_df, gzfile("spiegel_cont_urls_df.csv.gz"))
